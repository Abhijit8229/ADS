...
class GradeBook:
    ...
    def print_individual_grades(self, id):
        grades = [i for i in self.students if i.andrew_id == id]
        if grades:
            print(grades[0].get_grades_str())
        else:
            
            print(f"No student found with id: {id}")

    def print_grades_by_major(self, major):
        grades = [i for i in self.students if i.major == major]
    ...
    def print_current_grade(self, id):
        student = next((s for s in self.students if s.andrew_id == id), None)
        if student:
            
            print(f"Current letter grade for {id} is: {student.get_current_letter_grade()}")
        else:
            print(f"Error: No student with id {id} found.")

    def what_do_i_need(self, id, target_grade):
        student = next((s for s in self.students if s.andrew_id == id), None)
        if student:
            needed_score = student.what_do_i_need(target_grade)
            if needed_score == -1:
                print(f"{id} has already completed all assignments.")
            else:
                print(f"Student {id} needs an average of {needed_score:.2f} on the remaining work for a(n) {target_grade}")
        else:
            print(f"Error: No student with id {id} found.")
